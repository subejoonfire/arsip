...
<div class="bg-white p-6 rounded-lg shadow-md">
    <h2 class="text-2xl font-bold text-gray-800 mb-6"><?= esc($title ?? 'Daftar Surat Masuk') ?></h2>

    <div class="flex justify-between items-center mb-4">
        <a href="/admin/suratmasuk/tambah"
            class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium transition duration-300">
            <i class="fas fa-plus-circle mr-1"></i> Tambah Surat Masuk
        </a>
    </div>
            ...
            <tbody class="text-gray-700 text-sm font-light">
                ...
                foreach ($suratMasuk as $row) : ?>
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        ...
                        <td class="py-3 px-6 text-left"><?= esc($row['no_surat'] ?? '-') ?></td>
                        <td class="py-3 px-6 text-left"><?= esc($row['tgl_surat'] ?? '-') ?></td>
                        <td class="py-3 px-6 text-left"><?= esc($row['pengirim'] ?? '-') ?></td>
                        <td class="py-3 px-6 text-left"><?= esc($row['perihal'] ?? '-') ?></td>
                        <td class="py-3 px-6 text-left">
                            <?php if (!empty($row['file_lampiran'])) : ?>
                                <a href="<?= base_url('uploads/' . $row['file_lampiran']) ?>" target="_blank"
                        ...
                        <td class="py-3 px-6 text-center">
                            <div class="flex item-center justify-center">
                                <a href="<?= base_url('admin/suratmasuk/edit/' . $row['id']) ?>"
                                    class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110 text-blue-600">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="<?= base_url('admin/suratmasuk/delete/' . $row['id']) ?>"
